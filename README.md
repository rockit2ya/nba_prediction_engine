...
| File/Folder                  | Purpose                                                                                   |
|------------------------------|------------------------------------------------------------------------------------------|
| `nba_engine_ui.py`           | Main command-line interface for running the NBA analytics engine and interacting with users. Handles all user input, analysis, and logging. Now supports graceful Ctrl+C exit. |
| `nba_analytics.py`           | Core analytics logic, modeling, and hybrid data-fetching. Tries `nba_api` first, then Selenium, then Playwright, and falls back to cache or baseline if needed. |
| `nba_data_fetcher_advanced.py` | Selenium-based NBA.com advanced stats scraper. Used as a fallback if `nba_api` fails.    |
| `nba_data_fetcher_playwright.py` | Playwright-based NBA.com advanced stats scraper. Used as a fallback if both `nba_api` and Selenium fail. |
| `nba_data_fetcher.py`        | (Legacy/optional) General NBA stats fetcher, not used in the hybrid workflow.            |
| `fetch_all_data.sh`          | Bash script to robustly run all data fetchers and ensure `nba_stats_cache.json` is up to date. |
| `nba_stats_cache.json`       | Cached NBA team advanced stats (auto-generated/updated by fetchers and engine).           |
| `.stats_timestamp`           | Timestamp of the last successful cache update (auto-generated).                          |
| `bet_tracker_YYYY-MM-DD.csv` | Daily log of all bets, fair lines, edges, and Kelly calculations.                        |
| `game_schedule.csv`          | (Optional) Local schedule data for rest/fatigue calculations.                            |
| `late_scratch_checker.py`    | Script to check for last-minute injury/scratch news from NBA/ESPN before betting.        |
| `requirements.txt`           | Python dependencies for the project.                                                     |
| `fetch_all_data.log`         | Log file generated by `fetch_all_data.sh` with details of each fetcher run.              |
...
### Hybrid Data Fetching Workflow

- The engine first attempts to fetch live NBA stats using the `nba_api` Python package.
- If `nba_api` fails (due to API changes, rate limits, or network issues), it automatically runs the Selenium-based fetcher (`nba_data_fetcher_advanced.py`).
- If Selenium also fails, it tries the Playwright-based fetcher (`nba_data_fetcher_playwright.py`).
- If all live fetchers fail, it uses the last cached data (`nba_stats_cache.json`), and as a last resort, falls back to a baseline league average.
- The `.stats_timestamp` file is always updated when the cache is refreshed, so the UI displays the correct data age.

#### How to Update Data

- To ensure you have the latest stats, run:
	```bash
	bash fetch_all_data.sh
	```
	This will run all fetchers in order and update the cache.

- Or, simply run:
	```bash
	python nba_engine_ui.py
	```
	The engine will automatically use the hybrid fetching logic in the background.

#### Additional Tools

- `late_scratch_checker.py` can be run manually before betting to check for last-minute injury news.
- All logs and errors are written to `nba_data_fetcher.log` and `fetch_all_data.log` for troubleshooting.
