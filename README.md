...
| `update_results.py`              | Auto-fetches final scores from NBA API and updates bet tracker CSVs with WIN/LOSS results.                                                                                 |
...
5. **Post-Analysis**
   - Run `python post_mortem.py` and enter a date to review that day's results.
   - Shows win/loss record for high-signal bets, flags losses correlated with injuries or low edge, and summarizes win rate.
   - Note: Requires the `Result` column in the bet tracker CSV to be manually updated to `win` or `loss`.
...
| -------- | ---------------------------------------- | ---------------------------------------- |
| `Result` | `WIN`, `LOSS`, or `PENDING`              | Auto-populated by `update_results.py` or set manually |
